...
> 已计算，而且已写入结构化结果：`data/processed/strategy_ranked_extended.csv` 中当前 243 个样本均有 `apr`、`pnl_all_time_last`、`pnl_all_time_max_dd`、`pnl_all_time_stability` 与 `extended_score`。

| Rank | Strategy | APR | TVL | All-time PnL | Max Drawdown | Stability (std ΔPnL) | Extended Score |
|---:|---|---:|---:|---:|---:|---:|---:|
| 1 | `+convexity` | -5.08% | 494,966 | 62,592 | -38,514 | 24,591 | 0.5684 |
| 2 | `[ Systemic Strategies ] ♾️ HyperGrowth ♾️` | -7.96% | 6,831,503 | 1,919,330 | -1,317,750 | 926,030 | 0.5656 |
| 3 | `BTC/ETH CTA \| AIM` | -7.87% | 96,528 | 317 | -21,288 | 9,045 | 0.5517 |
| 4 | `Flagship by Fire Labs` | -1.90% | 17,831 | 2,916 | -49,532 | 32,203 | 0.5397 |
| 5 | `[ Sidelined ] Market Neutral 1` | -0.69% | 36,061 | 3,538 | -2,023 | 1,428 | 0.5372 |

### 核心指标说明

- `APR`：来自公开 stats 数据中的年化收益字段；README 中按百分比展示。
- `All-time PnL`：公开净值 / PnL 序列中的全周期末值。
- `Max Drawdown`：基于公开 PnL 序列计算的历史最大回撤。
- `Stability`：公开 PnL 序列变化的波动代理（std ΔPnL）；越低通常代表路径更平滑。
- `Extended Score`：在基础多因子评分上继续纳入 commission、withdrawable ratio、历史深度后的综合分数。
